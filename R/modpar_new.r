...
#' @param meth.type  type of Methylation Assay used. Either '27K' for Infinium HumanMethylation27K, '450K' for Infinium HumanMethylation450K or 'EPIC' for Infinium HumanMethylationEPIC. In this case also set row.ids.meth='cg_id'. If methylation data is already preprocessed and the data matrix contains methylation values per gene per sample set meth.type=""
...
#' @param row.ids.meth  type of rowname identifier in biomaRt database of DNA methylation matrix. If cg IDs are provided set row.ids.meth='cg_id'. Otherwise set row.ids.meth to  'ensembl_gene_id', 'refseq_mrna' or 'external_gene_name' for example. For details see https://www.bioconductor.org/packages//2.7/bioc/vignettes/biomaRt/inst/doc/biomaRt.pdf
